...
# ── Step 2: Scrape each speech page ──────────────────────────────────────────
...
def parse_speech(html: str) -> dict:
    ...
    return {"title": title, "speaker": speaker, "content": content, "footnote": footnote}
...
def scrape_speeches():
    ...
    while True:
        ...
        try:
            parsed = parse_speech(fetch_url(entry["url"]))
            result = {
                "date": entry["date"],
